...
import { deleteInterestById } from 'apiSdk/interests';
...
function SellerViewPage() {
  ...
  const interestHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('interest', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/interests/create?seller_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.interest?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/interests/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              ...
                              {hasAccess('interest', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    interestHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
