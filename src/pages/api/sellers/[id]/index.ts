...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'DELETE':
      return deleteSellerById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getSellerById() {
    const data = await prisma.seller.findFirst(convertQueryToPrismaUtil(req.query, 'seller'));
    return res.status(200).json(data);
  }

  async function updateSellerById() {
  ...
  async function deleteSellerById() {
    const data = await prisma.seller.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
